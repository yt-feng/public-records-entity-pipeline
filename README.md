...
Durable public extracts are committed under `data/`: `public_unique_people.csv.gz`, `public_gcc_unique_people.csv.gz`, `public_people_observations.csv.gz`, and `public_relationships.csv.gz`. The latest full XLSX is generated by Actions and uploaded as the workflow artifact; it is not committed because GitHub's per-file limit is 100 MB.
